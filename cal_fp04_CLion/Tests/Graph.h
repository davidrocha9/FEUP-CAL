...
bool Graph<T>::addEdge(const T &sourc, const T &dest, double w) {
    Vertex<T>* s1 = findVertex(sourc);
    Vertex<T>* d1 = findVertex(dest);
    if (s1 != NULL && d1 != NULL){
        s1->addEdge(d1,w);
        return true;
    }
...
bool Graph<T>::removeVertex(const T &in) {
    ...
    Vertex<T>* s1 = findVertex(in);
    vector<Vertex<T> *> v1;
    if (s1 != NULL){
        for (Vertex<T>* x : vertexSet){
            if (x != s1){
                x->removeEdgeTo(s1);
                v1.push_back(x);
            }
